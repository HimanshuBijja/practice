...
def solve_inequality(inequality_str):
    ...
    x = sp.symbols('x')
    solutions = sp.solve_univariate_inequality(inequality, x, domain=sp.S.Reals)
...
def curl(vector_field, variables):
    # Ensure we have exactly 3 variables for curl calculation
    if len(variables) != 3:
        raise ValueError("Curl requires exactly 3 variables (x, y, z)")
    if len(vector_field) != 3:
        raise ValueError("Curl requires exactly 3 components in the vector field")
...
if __name__ == "__main__":
    ...
    print("We are here to solve your complex problems :)")
    ...
    options = {
        ...
        '6': "Compute gradient of a function",
        '7': "Compute curl of a vector field",
    ...
    try:
        ...
        if choice == '1':
            ...
            print("Note: Curl requires exactly 3 components and 3 variables (x, y, z)")
            vector_field = []
            for i in range(3):
                component = input(f"Enter component {i+1} (e.g., x*y): ")
                vector_field.append(parse_expression(component))
            variables = input("Enter the variables (x y z): ").split()
            if len(variables) != 3:
                print("Error: Please provide exactly 3 variables")
            else:
                variables = [sp.symbols(var) for var in variables]
            ...
            print("Enter ODE using SymPy syntax. Example: Eq(Derivative(f(x), x, x) + f(x), 0)")
            equation_str = input("Enter the ODE: ")
